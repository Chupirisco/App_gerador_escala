     ...
     const [local, setLocal] = useState<Localidade[]>([]);
       ...
       useEffect(()=>{
         try{
           ...
           listarLocalidade().then((res)=> {setLocal(res)});                
       ...
       return (
        <div className="w-100 h-auto d-flex align-items-center flex-column my-5">
            ...
            <div className="card shadow-sm w-75 p-3 mt-5">
                <form className="row align-items-end g-2" onSubmit={cadastrar} >
                    <div className="col-md-15">
                        ...
                        <input type="text" placeholder="Nome do individuo" value={nome} onChange={(e) => setNome(e.target.value)} className="form-control"/>
                    </div>            
                     <div className="col-md-3">
                        <label className="form-label">Status</label>
                        <select value={status} onChange={(e) => setStatus(e.target.value)} className="form-select shadow-sm"> 
                    ...
                    {/* botões */}
                    <Link href={'/configuracao/individuo/'} className="btn btn-outline-danger text-black col-md-1 mx-2">Cancelar</Link>
           ...
           <div className="position-fixed end-0 bottom-0 p-3">
                <Notificacao mensagem={notMenssagem} onClose={() => setNot(false)} type={notTipo === "sucesso" ? 'sucesso' : 'erro'}></Notificacao>
           </div>
            }     
        </div>
    );
